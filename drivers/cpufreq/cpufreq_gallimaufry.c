 ...
 *           (C)  2012 Shane Douglas <jdkoreclipse@gmail.com>
...
#ifdef CONFIG_CPU_FREQ_GOV_GALLIMAUFRY_FLEXRATE
 ...
 * rate is 10 ms because the user needs immediate reaction from gallimaufry
...
#endif /* CONFIG_CPU_FREQ_GOV_GALLIMAUFRY_FLEXRATE */
			       ...
			       void *dummy)
{
	 ...
	 * from up-threshold - e load, the gallimaufry governor will react
	 * sampling_rate * 2.
	 *
...
static struct notifier_block gallimaufry_qos_dvfs_lat_nb = {
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	err = pm_qos_add_notifier(PM_QOS_DVFS_RESPONSE_LATENCY,
			    &gallimaufry_qos_dvfs_lat_nb);
	if (err)
		return err;

	err = cpufreq_register_governor(&cpufreq_gov_gallimaufry);
	if (err) {
...
static unsigned int max_duration =
		(CONFIG_CPU_FREQ_GOV_GALLIMAUFRY_FLEXRATE_MAX_DURATION);
...
static int cpufreq_gallimaufry_flexrate_do(struct cpufreq_policy *policy,
					bool now)
{
	unsigned int cpu = policy->cpu;
	bool using_gallimaufry;
	struct cpu_dbs_info_s *dbs_info = &per_cpu(od_cpu_dbs_info, cpu);
	...
	if (now) {
		...
		mutex_lock(&dbs_mutex);
		using_gallimaufry = dbs_enable && !strncmp(policy->governor->name, "gallimaufry", 8);
		mutex_unlock(&dbs_mutex);

		if (!using_gallimaufry)
			return 0;
...
int cpufreq_gallimaufry_flexrate_request(unsigned int rate_us,
...
out:
	...
	if (err > 0) {
		...
		/* TODO: For every CPU using GALLIMAUFRY */
		err = cpufreq_gallimaufry_flexrate_do(cpufreq_cpu_get(cpu), now);
	}
	mutex_unlock(&flex_mutex);
	return err;
}
EXPORT_SYMBOL_GPL(cpufreq_gallimaufry_flexrate_request);
				      ...
				      const char *buf, size_t count)
{
	...
	ret = cpufreq_gallimaufry_flexrate_request(rate, sysfs_duration);
	if (ret)
		return ret;
	return count;
}
...
static ssize_t show_flexrate_request(struct kobject *a, struct attribute *b,
				     char *buf)
{
	return sprintf(buf, "Flexrate decreases CPUFreq gallimuafry governor's polling rate temporaily.\n"
			    "Usage Example:\n"
			    "# echo 8 > flexrate_duration\n"
			    "# echo 10000 > flexrate_request\n"
			    "With the second statement, Gallimaufry polls with 10ms(10000us) interval 8 times.\n"
